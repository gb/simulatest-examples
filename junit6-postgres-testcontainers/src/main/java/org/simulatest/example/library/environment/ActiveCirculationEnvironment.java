 ...
 *
 * <p>The one dialect difference vs. the H2 demo: Postgres uses
 * {@code CURRENT_DATE + N} rather than H2's {@code DATEADD('DAY', N, CURRENT_DATE)}.
 */
...
public final class ActiveCirculationEnvironment implements Environment {
	...
	public void run() {
		...
		// Postgres: `date + integer` shifts by N days. No DATEADD needed.
		LibraryDatabase.execute("INSERT INTO hold VALUES (1, 1, 7, CURRENT_DATE - 2, 'ACTIVE')");
		LibraryDatabase.execute("INSERT INTO hold VALUES (2, 3, 8, CURRENT_DATE - 1, 'ACTIVE')");
	}
	...
	private void insertLoan(int id, int copyId, int memberId, int checkoutDaysAgo, int dueDaysFromNow) {
		LibraryDatabase.execute(
			...
			"CURRENT_DATE + " + checkoutDaysAgo + ", " +
			"CURRENT_DATE + " + dueDaysFromNow + ", NULL)");
	}
