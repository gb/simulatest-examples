...
A plain-Java demo built around a tiny library domain (books, members, branches, loans). It's the shortest path to seeing the three things Simulatest actually does: an environment tree, the Insistence Layer savepoint stack, and the JUnit 5 TestEngine.

## Stack

Java 17 · JUnit 5 · H2 · raw JDBC. No Spring, no ORM, nothing else.
...
- **JUnit 5 TestEngine.** Tests run with plain `@Test` annotations; the Simulatest engine is auto-discovered via `ServiceLoader`.
